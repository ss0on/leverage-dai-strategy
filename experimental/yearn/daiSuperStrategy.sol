...
import "./interface/dydxFlashLoan.sol";
import "./interface/ComptrollerI.sol";
import "./interface/CErc20I.sol";
import "./interface/IUni.sol";

// Supplies DAI on Compound and borrows an additional amount of DAI to maximize COMP earnings.
// Flashloans are used to obtain additional DAI from dYdX in order to gain additional leverage and boost the APY.
// Earned COMP is harvested and sold for more DAI and re-deposited into the vault.
// https://excalidraw.com/#json=6232024662671360,MgpZcsnwAh98RPKNghvmTw

/********************
 *
 *   A lender optimization strategy for any erc20 asset
...
contract Strategy is BaseStrategy, DydxFlashloanBase, ICallee {
  ...
  function estimatedTotalAssets() public view override returns (uint256) {
    ...
    // Use touch price. it doesn't matter if we are wrong as this is not used for decision making
    ...
    return
      want.balanceOf(address(this)).add(deposits).add(conservativeWant).sub(
        borrows
      );
  }
  ...
  //WARNING does not include compounding so the estimate becomes more inaccurate the further ahead we look
  //equation. Compound doesn't include compounding for most blocks
  //((deposits*collateralThreshold - borrows) / (borrows*borrowRate - deposits*collateralThreshold*interstate));
  function getblocksUntilLiquidation() public view returns (uint256) {
    ...
    uint256 collateralisedDeposit1 = deposits.mul(collateralFactorMantissa).div(
      1e18
    );
  ...
  //WARNING - this returns just the balance at last time someone touched the cToken token. Does not accrue interest in between
  ...
  /*
   * A core method.
   * Called at the beginning of harvest before providing report to owner
  ...
  function adjustPosition(uint256 _debtOutstanding) internal override {
    ...
    if (_wantBal < _debtOutstanding) {
      //this is graceful withdrawal. don't use backup
   ...
   * Returns amount we were able to withdraw. no tall if user has some balance left
   *
  ...
  function _withdrawSome(uint256 _amount) internal returns (bool notAll) {
    ...
    //If there is no deficit we don't need to adjust position
    ...
    if (deficit && position > minWant) {
      ...
      //if we are not in deficit we don't need to do flash loan
    ...
    if (
      collateralTarget == 0 && want.balanceOf(address(this)) > borrowBalance
    ) {
  ...
  //lets leave
  //if we can't deleverage in one go set collateralFactor to 0 and call harvest multiple times until de-levered
    ...
    returns (uint256 _amountFreed)
  {
    ...
    //we want to revert if we can't liquidate-all
    if (!forceMigrate) {
